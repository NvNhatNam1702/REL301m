...
class HybridPodDeliveryEnv:
    ...
    def __init__(self, seed=None, fixed_pod_positions=None):
        ...
        # Pod management
        self.num_total_pods = 36  # Total pods in environment
    ...
    def _init_visualization(self):
        ...
        if self.window is None:
            self.window = tk.Tk()
            self.window.title("Hybrid Pod Collection & Delivery Environment (Large)")
            ...
            self.info_label = tk.Label(
                self.window,
                text="Mode: COLLECTING | Steps: 0 | Collected: 0/5 | Delivered: 0",
                font=("Arial", 12)
            )
    ...
    def _handle_collection_step(self, action):
        ...
        if self.robot_pos in self.pod_positions and self.robot_pos not in self.collected_pods:
            ...
            reward = 0.0  # No reward for collecting a pod
        else:
    ...
    def _handle_delivery_step(self):
        ...
        if self.path_index < len(self.current_path):
            ...
            if self.robot_pos == self.delivery_point:
                ...
                self.mode = "COLLECTING"
                return 0.0, self._check_done()  # No reward for successful delivery
    ...
    def get_uncollected_pods(self):
        """Return a list of uncollected pod positions (row, col)."""
        return [pod for pod in self.pod_positions if pod not in self.collected_pods]

    def step_select_pod(self, pod_index):
        """Take a step by selecting a pod to collect (by index in uncollected pods list)."""
        uncollected_pods = self.get_uncollected_pods()
        if not uncollected_pods or pod_index < 0 or pod_index >= len(uncollected_pods):
            # Invalid action
            return self.get_state(), -1.0, True, {"error": "Invalid pod selection"}
        pod_pos = uncollected_pods[pod_index]
        # Plan path to pod
        path = self._plan_path(self.robot_pos, pod_pos)
        if not path:
            return self.get_state(), -1.0, True, {"error": "No path to pod"}
        # Move along the path
        for pos in path[1:]:  # skip current position
            ...
            if self.render_enabled:
                self.render()
        # Collect the pod
        self.collected_pods.add(pod_pos)
        reward = 0.0  # No reward for collecting a pod
        done = self._check_done() or (self.current_steps >= self.max_steps)
        # If enough pods collected, switch to delivery mode
        if len(self.collected_pods) >= self.pods_per_delivery:
            self.mode = "DELIVERING"
            self.current_path = self._find_path_to_delivery()
            if not self.current_path:
                self.current_path = self._find_alternative_delivery_path()
            self.path_index = 0
        return self.get_state(), reward, done, {"pod_collected": pod_pos}

    def step_select_pod_batch(self, pod_indices):
        """Take a step by selecting a batch of 3 pods to collect (by indices in uncollected pods list)."""
        uncollected_pods = self.get_uncollected_pods()
        if len(uncollected_pods) < 3 or any(i < 0 or i >= len(uncollected_pods) for i in pod_indices) or len(set(pod_indices)) != 3:
            return self.get_state(), -1.0, True, {"error": "Invalid pod batch selection"}
        batch_pods = [uncollected_pods[i] for i in pod_indices]
        total_steps = 0
        reward = 0.0
        current_pos = self.robot_pos
        # Reward for batch closeness
        batch_dists = [abs(a[0]-b[0]) + abs(a[1]-b[1]) for i, a in enumerate(batch_pods) for b in batch_pods[i+1:]]
        batch_avg_dist = sum(batch_dists) / len(batch_dists)
        reward += 0.05 * (10 - batch_avg_dist)  # Positive reward for close batches
        # Collect each pod in order
        for pod in batch_pods:
            path = self._plan_path(current_pos, pod)
            if not path or len(path) < 2:
                return self.get_state(), -1.0, True, {"error": "No path to pod in batch"}
            for pos in path[1:]:
                self.robot_pos = pos
                self.current_steps += 1
                total_steps += 1
                if self.render_enabled:
                    self.render()
            self.collected_pods.add(pod)
            # No reward for collecting a pod
            current_pos = pod
        # After collecting batch, switch to delivery mode
        self.mode = "DELIVERING"
        delivery_path = self._plan_path(self.robot_pos, self.delivery_point)
        delivery_steps = 0
        if delivery_path and len(delivery_path) > 1:
            for pos in delivery_path[1:]:
                self.robot_pos = pos
                self.current_steps += 1
                delivery_steps += 1
                if self.render_enabled:
                    self.render()
            # No reward for delivery
            for pod in batch_pods:
                self.pod_positions.remove(pod)
            self.delivered_pods += len(batch_pods)
            self.collected_pods.clear()
            self.mode = "COLLECTING"
        else:
            # Failed to deliver
            reward -= 1.0
        total_steps += delivery_steps
        reward -= 0.01 * total_steps
        # Optional: penalty for future pod dispersion (average pairwise distance)
        remaining = self.get_uncollected_pods()
        if len(remaining) > 1:
            dists = [abs(a[0]-b[0]) + abs(a[1]-b[1]) for i,a in enumerate(remaining) for b in remaining[i+1:]]
            avg_dist = sum(dists) / len(dists)
            reward -= 0.01 * avg_dist
        done = self._check_done() or (self.current_steps >= self.max_steps)
        return self.get_state(), reward, done, {"pods_collected": batch_pods, "steps": total_steps, "delivery_steps": delivery_steps}

    def _plan_path(self, start, goal):
        """A* pathfinding from start to goal, avoiding uncollected pods as obstacles."""
        # This is similar to _find_path_to_delivery, but for arbitrary pod positions
        if start == goal:
            return [start]
        frontier = []
        heapq.heappush(frontier, (0, start))
        ...
        while frontier:
            ...
            if current == goal:
                break
            for next_pos in self._get_neighbors(current):
                # Avoid uncollected pods except the goal pod
                if next_pos in self.pod_positions and next_pos not in self.collected_pods and next_pos != goal:
                    continue
                new_cost = cost_so_far[current] + 1
                if next_pos not in cost_so_far or new_cost < cost_so_far[next_pos]:
                    cost_so_far[next_pos] = new_cost
