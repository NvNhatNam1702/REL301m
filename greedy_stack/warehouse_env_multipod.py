...
class WarehouseEnvMultiPod:
    ...
    def _generate_random_pods(self):
        # For each cell (except workstations and delivery), assign 0-3 pods randomly, but with lower probability
    ...
    def step(self, action):
        ...
        if (0 <= new_pos[0] < self.grid_height and 0 <= new_pos[1] < self.grid_width):
            self.robot_pos = new_pos
            # If on a pod cell, deliver one pod
            if self.target_pod and self.robot_pos == self.target_pod and self.grid[self.robot_pos] > 0:
                self.mark_pod_delivered(self.robot_pos)
                reward += 1.0
                # Select new target if pods remain
                self.target_pod = self._select_new_target()
            # Small reward for moving closer to target
            if self.target_pod:
                old_dist = abs(self.robot_pos[0] - dr - self.target_pod[0]) + abs(self.robot_pos[1] - dc - self.target_pod[1])
                new_dist = abs(self.robot_pos[0] - self.target_pod[0]) + abs(self.robot_pos[1] - self.target_pod[1])
                reward += 0.1 * (old_dist - new_dist)
        else:
            reward = -0.1  # Penalty for invalid move
        done = self.all_pods_delivered()
        self.render()
